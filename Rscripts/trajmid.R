...
for(layers in c("EFGHJP")){ #Loop over models, order determines what goes over/under in plot
  ...
  for (epoch in c(1:nepoch)){
    ...
    epochlist <- append(epochlist,sprintf("%02d",epoch+40)) #Starting from epoch 41
...
#        xlab="Generation",ylab="Mismatch",cex.lab=1.5)
#legend("topright",legend=modelvec[bxpindex],col=colvec.s[bxpindex],title="Model",lty=1)
#dev.off()


#Analyze individual trajectories close to transition point
png(sprintf("errtraj_%d.png",denv),width=2250,height=2250,units="px",pointsize=12,res=300)
#make main plot first
par(fig=c(0,1,0,1)) #Specify NDC coordinates for main plot
matplot(mat.err[,2:ncol(mat.err)],col=c(1:nepoch),type="l",lty=c(1:nepoch),
        xlab="Generation",ylab="Mismatch",sub=sprintf("(%s environmental change)",str.pdenv),cex.lab=1.5)
legend("topright",legend=epochlist,col=c(1:nepoch),title="Epoch",lty=c(1:nepoch))
#make inset plot for first 10 gens to check for MLE VS PLE
par(fig=c(0.2,0.8,0.4,0.95),new=T)
matplot(mat.err[1:10,2:ncol(mat.err)],ylim=c(0.7,1.0),
        col=c(1:nepoch),type="l",lty=c(1:nepoch),xlab="",ylab="") #Remove axis labels
dev.off()
